// Path B direct-push helper for the Claude Code hook.
//
...
// fire-and-forget so the hook exit code is never affected by Cloud
// reachability.
//
// trace_id for Claude Code hook invocations comes from the OTEL_TRACE_ID
// env var (read via @jamjet/cloud's readTraceparent). Claude Code may set
// this when running an instrumented session; v0.1 doesn't fabricate a
// trace_id when absent.
...
/** Read the OTel trace_id from environment (OTEL_TRACE_ID), if set. */
export function traceIdFromEnv(): string | undefined {
  return readTraceparent()?.trace_id
}
